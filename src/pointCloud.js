...
export class PointCloud {
  ...
  _build() {
    ...
    const baseColor   = new THREE.Color(0x00ffe0)   // cyan-teal
    const dimColor    = new THREE.Color(0x004d45)   // dark teal for variety
    const hoverColor  = new THREE.Color(0xffffff)

    // Use seeded random for reproducible layouts
    let rs = p.seed
    function srnd() {
    ...
    for (let i = 0; i < p.pointCount; i++) {
      ...
      // Noise displacement to make it organic / clumped
      ...
      // Per-point drift phase offsets
      this.driftOffsets[i * 3]     = srnd() * 100
      this.driftOffsets[i * 3 + 1] = srnd() * 100
      this.driftOffsets[i * 3 + 2] = srnd() * 100

      // Color: base with slight variation
  ...
  update(time) {
    ...
    // Rebuild line positions to follow drifting points
  ...
  setHovered(index) {
    ...
    // Find connected point indices for the hovered point
  ...
  applyParam(key, value) {
    ...
    switch (key) {
      case 'pointSize':
        this.pointsMesh.material.size = value
        break
      case 'lineOpacity':
